...
const EventDescription = ({ item }: { item: FeedItem }) => {
    if (item.source === 'manual') {
        return (
            <span>
                Added proof: <strong>{item.title}</strong>
            </span>
        )
    }
    ...
    switch (type) {
        case 'PushEvent':
            ...
            return (
                <span>
                    Pushed {commitCount} commit{commitCount > 1 ? 's' : ''} to <strong>{repoName}</strong>
                </span>
            )
        case 'PullRequestEvent':
            return (
                <span>
                    {payload.action} PR in <strong>{repoName}</strong>
                </span>
            )
        case 'WatchEvent':
            return (
                <span>
                    Starred <strong>{repoName}</strong>
                </span>
            )
        case 'CreateEvent':
            return (
                <span>
                    Created {payload.ref_type} <strong>{repoName}</strong>
                </span>
            )
        case 'IssueCommentEvent':
            return (
                <span>
                    Commented on issue in <strong>{repoName}</strong>
                </span>
            )
        default:
            return (
                <span>
                    Performed {type} on <strong>{repoName}</strong>
                </span>
            )
    }
}
